...
def generate_wg_config(
    ...
    The gateway_endpoint is written verbatim. If it needs to be a DNS name
    (so the kernel can re-resolve when an NLB IP rotates), the server is
    responsible for returning it as DNS in the registration response.
    ...
    Args:
        ...
        gateway_endpoint: Gateway's endpoint as returned by the platform
            (e.g., "tunnel.ares.example.com:51820" or "1.2.3.4:51820")
...
def generate_and_write_config() -> Optional[Path]:
    ...
    if missing:
        logger.error(
            "Cannot generate WireGuard config — missing required fields from "
            "registration: %s. Re-register the agent to recover.",
            ", ".join(missing),
        )
        return None
