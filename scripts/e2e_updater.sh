...
from updater import dockerd, main
try:
    main._tick(dockerd)
...
ARES_UPDATE_CONTAINER="$NAME" ARES_UPDATE_IMAGE_REPO=alpine ARES_UPDATE_REQUIRE_SIGNATURE=false ARES_UPDATE_TARGET_FILE="$T/t.json" tick
...
fresh_agent alpine:3.18
printf '{"version":"3.19"}' >"$T/t.json"
ARES_UPDATE_CONTAINER="$NAME" ARES_UPDATE_IMAGE_REPO=alpine ARES_UPDATE_REQUIRE_SIGNATURE=true ARES_UPDATE_TARGET_FILE="$T/t.json" tick
